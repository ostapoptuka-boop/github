...
// ==========================================
let trustMainBalance = parseFloat(localStorage.getItem("trustMainBalance")) || 0.0;
...
function checkAuthUI() {
  if (isAuth) {
    ...
    if (savedName) {
      ...
      const initials = parts.length >= 2 ? (parts[0][0] + parts[1][0]).toUpperCase() : savedName.slice(0, 2).toUpperCase();
...
function switchDepTab(tab) {
  ...
  if (tab === "crypto") {
    ...
    btnCrypto.classList.add("active-dep-tab");
    btnCard.classList.remove("active-dep-tab");
  } else {
    formCrypto.style.display = "none";
    formCard.style.display = "block";
    btnCard.classList.add("active-dep-tab");
    btnCrypto.classList.remove("active-dep-tab");
  }
}
...
function executeTrade(type) {
  ...
  if (type === "buy") {
    ...
    if (trustPortfolio[currentTradeCoin] < 0.000001) {
      delete trustPortfolio[currentTradeCoin];
      delete trustBuyPrices[currentTradeCoin];
    }
    trustMainBalance += usdtInput;
    localStorage.setItem("trustMainBalance", trustMainBalance);
    localStorage.setItem("trustPortfolio", JSON.stringify(trustPortfolio));
...
function renderPortfolio() {
  ...
  if (!hasItems)
    list.innerHTML =
      '<div style="padding:2.5rem;text-align:center;color:#475569;"><i class="fa-solid fa-chart-pie" style="font-size:2rem;margin-bottom:12px;display:block;opacity:0.3;"></i>У вас пока нет активов.<br><span style="font-size:0.85rem;">Перейдите в Маркет для покупки.</span></div>';
...
function executeSellModal() {
  ...
  errEl.style.display = "none";
  if (!amt || amt <= 0) {
    errEl.textContent = "Введите количество для продажи!";
    errEl.style.display = "block";
    return;
  }
  if (!trustPortfolio[coin] || trustPortfolio[coin] < amt) {
    errEl.textContent = "Недостаточно " + coin + " на балансе!";
    errEl.style.display = "block";
    return;
  }
  const price = chartHistory[coin] && chartHistory[coin].length > 0 ? chartHistory[coin][chartHistory[coin].length - 1] : 0;
  ...
  if (trustPortfolio[coin] < 0.000001) {
    delete trustPortfolio[coin];
    delete trustBuyPrices[coin];
  }
...
const getFakeHistoryChart = (basePrice) => {
  var arr = [];
  var p = basePrice * (0.7 + Math.random() * 0.2);
  for (var i = 0; i < 200; i++) {
...
chartHistory["BTC"] = getFakeHistoryChart(64000);
chartHistory["ETH"] = getFakeHistoryChart(3400);
chartHistory["BNB"] = getFakeHistoryChart(600);
chartHistory["XRP"] = getFakeHistoryChart(0.5);
chartHistory["USDC"] = getFakeHistoryChart(1);
chartHistory["SOL"] = getFakeHistoryChart(145);
chartHistory["TRX"] = getFakeHistoryChart(0.12);
chartHistory["DOGE"] = getFakeHistoryChart(0.15);
chartHistory["PEPE"] = getFakeHistoryChart(0.00001);
chartHistory["SHIB"] = getFakeHistoryChart(0.00002);
chartHistory["WIF"] = getFakeHistoryChart(2.5);
chartHistory["FLOKI"] = getFakeHistoryChart(0.00015);
...
function tickTrsPrice() {
  ...
  var maxUp = trsAdminPrice * 1.1;
  var maxDn = trsAdminPrice * 0.9;
...
function updateTrsFromAdmin() {
  ...
  trsStoredChart = trsStoredChart.map(function (v) {
    return v * ratio;
  });
  ...
  if (typeof candleCache !== "undefined") {
    for (var k in candleCache) {
      if (k.indexOf("TRS") === 0) delete candleCache[k];
    }
  }
}
  ...
  if (trsRow && chartHistory["TRS"] && chartHistory["TRS"].length > 2) {
    ...
    if (cw) {
      ...
      if (svg) {
        ...
        var valid = h.filter(function (v) {
          return v > 0;
        });
        if (valid.length > 1) {
          var mn = Math.min.apply(null, valid),
            mx = Math.max.apply(null, valid);
...
function startMarketLiveFeed() {
  const validStreams = marketSymbols
    .filter((s) => s !== "TRSUSDT")
...
function submitWithdrawal() {
  ...
  if (!address) {
    errEl.textContent = "Введите адрес кошелька!";
    errEl.style.display = "block";
    return;
  }
  if (!amount || amount < 10) {
    errEl.textContent = "Минимальная сумма вывода — 10 USDT!";
    errEl.style.display = "block";
    return;
  }
  if (amount > trustMainBalance) {
    errEl.textContent = "Недостаточно средств на балансе!";
    errEl.style.display = "block";
    return;
  }
...
function handleAvatarUpload(event) {
  ...
  if (file.size > 5 * 1024 * 1024) {
    showCustomAlert("Файл слишком большой", "Максимальный размер файла — 5MB.", "error");
    return;
  }
...
function renderHistory() {
  ...
  if (filtered.length === 0) {
    body.innerHTML =
      '<div style="padding:50px 20px;text-align:center;"><i class="fa-solid fa-clock-rotate-left" style="font-size:2.5rem;color:#1e293b;display:block;margin-bottom:14px;"></i><div style="color:#475569;font-size:0.9rem;">Нет транзакций в этой категории</div></div>';
    return;
  }
    ...
    .map((tx, i) => {
      ...
      if (tx.type === "withdraw") {
        if (tx.decided) {
          ...
          for (var wi = wdReqs.length - 1; wi >= 0; wi--) {
            if (tx.amount && Math.abs((wdReqs[wi].amount || 0) - (tx.amount || 0)) < 0.01) {
              matchReq = wdReqs[wi];
              break;
            }
          }
      ...
      if (tx.type === "withdraw") {
        if (wdStatus === "approved")
          statusBadge =
            '<span style="background:rgba(74,222,128,0.1);color:#4ade80;border:1px solid rgba(74,222,128,0.2);border-radius:6px;padding:2px 8px;font-size:0.68rem;font-weight:700;">Выполнено</span>';
        else if (wdStatus === "rejected")
          statusBadge =
            '<span style="background:rgba(248,113,113,0.1);color:#f87171;border:1px solid rgba(248,113,113,0.2);border-radius:6px;padding:2px 8px;font-size:0.68rem;font-weight:700;">Отклонён</span>';
        else
          statusBadge =
            '<span style="background:rgba(148,163,184,0.1);color:#94a3b8;border:1px solid rgba(148,163,184,0.25);border-radius:6px;padding:2px 8px;font-size:0.68rem;font-weight:700;">На проверке</span>';
        // Override icon to gray for pending
        if (wdStatus === "pending") {
          cfg = Object.assign({}, cfg, { color: "#94a3b8", bg: "rgba(148,163,184,0.1)" });
        }
      } else {
        statusBadge =
          '<span style="background:rgba(74,222,128,0.1);color:#4ade80;border:1px solid rgba(74,222,128,0.2);border-radius:6px;padding:2px 8px;font-size:0.68rem;font-weight:700;">Выполнено</span>';
...
function submitWModal() {
  ...
  if (!address || address.length < 20) {
    errEl.textContent = "Введите корректный адрес кошелька (мин. 20 символов)!";
    errEl.style.display = "block";
    return;
  }
  ...
  if (!validAddr) {
    errEl.textContent = "Неверный формат адреса для сети " + network + "!";
    errEl.style.display = "block";
    return;
  }
  if (!amount || amount < 10) {
...
function refreshAdminStats() {
  ...
  var wdPending = wdRequests.filter(function (r) {
    return r.status === "pending";
  });
  if (statW) statW.textContent = wdPending.length;
  var wdTabBadge = document.getElementById("wdTabBadge");
  if (wdTabBadge) {
    wdTabBadge.style.display = wdPending.length > 0 ? "flex" : "none";
    wdTabBadge.textContent = wdPending.length;
  }
...
function adminAddBalance() {
  ...
  if (!val || val <= 0) {
    adminLog("Ошибка: введите сумму > 0", "red");
    return;
  }
...
function adminSubBalance() {
  ...
  if (val > trustMainBalance) {
    adminLog("Ошибка: сумма списания > баланса", "red");
    return;
  }
  trustMainBalance -= val;
  localStorage.setItem("trustMainBalance", trustMainBalance);
...
function adminSetBalance() {
  ...
  if (val === undefined || isNaN(val) || val < 0) {
    adminLog("Ошибка: некорректное значение", "red");
    return;
  }
  trustMainBalance = val;
  localStorage.setItem("trustMainBalance", trustMainBalance);
...
function saveQuickEdit() {
  ...
  if (_quickEditField === "balance") {
    ...
    try {
      ...
      localStorage.setItem("trustPortfolio", JSON.stringify(obj));
      logAdminAction("Портфель обновлён");
    } catch (e) {
      showCustomAlert("Ошибка", "Некорректный формат JSON.", "error");
      return;
    }
  }
  closeQuickModal();
...
function showAdminUserAssets() {
  ...
  var coins = Object.keys(portfolio).filter(function (c) {
    return portfolio[c] > 0.000001;
  });
  if (!coins.length) {
  ...
  var html = coins
    .map(function (c) {
      ...
      var iconHtml = iconUrl
        ...
        : '<div style="width:28px;height:28px;border-radius:50%;background:linear-gradient(135deg,#1e3a6e,#2563eb);display:flex;align-items:center;justify-content:center;font-weight:800;font-size:0.7rem;color:#fff;">' +
          c.charAt(0) +
          "</div>";
      return (
        '<div style="display:flex;align-items:center;gap:12px;padding:10px 14px;background:rgba(0,0,0,0.25);border-radius:10px;border:1px solid rgba(255,255,255,0.04);">' +
        iconHtml +
        '<div style="flex:1;"><div style="color:#fff;font-weight:700;font-size:0.9rem;">' +
        c +
        '</div><div style="color:#64748b;font-size:0.75rem;">' +
        amt +
        ' монет</div></div><div style="text-align:right;"><div style="color:#4ade80;font-weight:700;">$' +
        val +
        "</div></div></div>"
      );
    })
...
function showAdminUserAirdrops() {
  ...
  var html = completed
    .map(function (a) {
      return (
        '<div style="display:flex;align-items:center;gap:12px;padding:10px 14px;background:rgba(0,0,0,0.25);border-radius:10px;border:1px solid rgba(255,255,255,0.04);"><div style="width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,rgba(168,85,247,0.2),rgba(99,102,241,0.15));display:flex;align-items:center;justify-content:center;font-size:1.1rem;flex-shrink:0;">' +
        a.icon +
        '</div><div style="flex:1;"><div style="color:#fff;font-weight:700;font-size:0.88rem;">' +
        a.title +
        '</div><div style="color:#64748b;font-size:0.72rem;">ID: ' +
        a.id +
        '</div></div><div style="color:#4ade80;font-weight:800;font-size:0.9rem;">+$' +
        a.reward +
        "</div></div>"
      );
    })
...
function renderAdminUsers() {
  ...
  const users = getAdminUserData().filter((u) => !search || u.id.toString().includes(search) || u.name.toLowerCase().includes(search));
  ...
  list.innerHTML = users
    .map((u) => {
      ...
      const verBadge = u.isVerified
        ? `<span style="background:rgba(74,222,128,0.12);color:#4ade80;border:1px solid rgba(74,222,128,0.3);border-radius:10px;padding:1px 6px;font-size:0.65rem;font-weight:700;">✔ Верф.</span>`
        : "";
...
function showAdminUserDetail(userId) {
  ...
  if (u.avatar) {
    ...
    avatarEl.style.background = "linear-gradient(135deg,#2563eb,#4f46e5)";
...
function renderAirdropsUI() {
  ...
  for (const [id, data] of Object.entries(airdropsData)) {
    ...
    if (taskStatus === "in_progress") {
      ...
      badgeClass = "status-moderation";
      badgeText = "На модерации";
...
function openAirdropDetails(id) {
  ...
  if (data.iconUrl) {
    document.getElementById("adModalIcon").innerHTML =
      `<img src="${data.iconUrl}" style="width:100%;height:100%;object-fit:cover;border-radius:inherit;" onerror="this.parentElement.textContent='${data.icon}'">`;
  }
...
function renderErrorLog() {
  ...
  if (!minerInitComplete()) {
    ...
    list.innerHTML =
      '<div style="color:#f59e0b;font-family:monospace;font-size:0.7rem;font-weight:700;letter-spacing:1px;margin-bottom:10px;padding-bottom:8px;border-bottom:1px solid rgba(245,158,11,0.15);">' +
    ...
    minerErrors.forEach(function (e, i) {
      ...
      list.innerHTML +=
        ...
        e.msg +
        "</div>" +
        '<button class="error-fix-btn" id="fixBtn' +
        i +
        '" onclick="applyInitFix(' +
        i +
        ')"' +
        (isFixed ? " disabled" : "") +
        ">" +
  ...
  if (!trustActiveError) {
    list.innerHTML =
      '<div style="color:rgba(16,185,129,0.6);font-family:monospace;font-size:0.72rem;text-align:center;padding:20px 0;">' +
  ...
  var idx = minerErrors.findIndex(function (x) {
  ...
  list.innerHTML =
    ...
    '<div class="error-code" style="font-size:0.85rem;">' +
    e.code +
    "</div>" +
    '<div class="error-desc" style="color:#e2e8f0;margin:6px 0 10px;">' +
    e.msg +
    "</div>" +
    '<button class="error-fix-btn" id="fixBtnActive" onclick="applyFix(' +
    idx +
    ')" ' +
    ...
    '▶ APPLY FIX<br><span style="font-size:0.65rem;opacity:0.7;">$ ' +
    e.fix +
    "</span>" +
    "</button>" +
    "</div>";
...
function bootMiner() {
  ...
  setTimeout(function () {
    writeLog("[BOOT] Loading kernel modules... OK", "#4ade80");
  }, 300);
  setTimeout(function () {
    writeLog("[BOOT] Detecting hardware... OK", "#4ade80");
  }, 700);
  setTimeout(function () {
    var algoNames = { sha256: "SHA-256", ethash: "Ethash", randomx: "RandomX", scrypt: "Scrypt" };
    var poolNames = { eu: "Neon Pool EU", us: "Neon Pool US", asia: "Neon Pool ASIA", solo: "Solo Mining" };
...
function startMiningLoop() {
  ...
  writeLog(
    "[NODE] · Algo: " + algoNames[(minerCfg && minerCfg.algo) || "sha256"] + " | Hashrate: ~" + effectiveHash.toFixed(0) + " MH/s | Pool: " + poolNames[(minerCfg && minerCfg.pool) || "eu"],
    "#4ade80",
  );
...
function saveArbCode() {
  ...
  if (!code) {
    showArbStatus("Введите код!", "#f87171");
    return;
  }
...
function kycNextStep(step) {
  ...
  for (var i = 1; i <= 4; i++) {
    ...
    if (line) {
      line.style.background = i < step ? "linear-gradient(90deg,#4ade80,#22c55e)" : i === step ? "linear-gradient(90deg,#6366f1,rgba(100,116,139,0.3))" : "rgba(100,116,139,0.3)";
    }
  }
}
...
function kycValidateStep2() {
  ...
  if (!ln.trim() || !fn.trim() || !dob) {
    [document.getElementById("kycLastName"), document.getElementById("kycFirstName"), document.getElementById("kycDob")].forEach(function (f) {
      if (f && !f.value.trim()) {
        ...
        setTimeout(function () {
          if (f) {
            f.style.borderColor = "rgba(100,116,139,0.3)";
  ...
  if (age < 18) {
    ...
    if (dobEl) {
      dobEl.style.borderColor = "#f87171";
      dobEl.style.boxShadow = "0 0 8px rgba(248,113,113,0.2)";
    }
    ...
    setTimeout(function () {
      ...
      if (idx + 1 < steps.length) {
        ...
        if (next) {
          ...
          next.innerHTML =
            '<i class="fa-solid fa-circle-notch fa-spin" style="color:#6366f1;font-size:0.9rem;"></i><span style="color:#94a3b8;font-size:0.82rem;">' +
            (["Сохранение данных...", "Проверка формата...", "Отправка заявки..."][idx + 1] || "...") +
            "</span>";
        }
      }
    }, s.delay);
...
function adminRenderKyc() {
  ...
  var displayList = kycShowProcessed
    ? list
      ...
      var btns =
        r.status === "pending"
          ? '<div style="display:flex;gap:8px;margin-top:14px;">' +
            '<button onclick="adminKycDecide(' +
            realIdx +
            ',\'approved\')" style="flex:1;background:linear-gradient(135deg,rgba(74,222,128,0.15),rgba(16,185,129,0.1));color:#4ade80;border:1px solid rgba(74,222,128,0.35);border-radius:10px;padding:9px;font-weight:800;font-size:0.82rem;cursor:pointer;transition:0.2s;" onmouseover="this.style.background=\'rgba(74,222,128,0.25)\'" onmouseout="this.style.background=\'rgba(74,222,128,0.15)\'"><i class="fa-solid fa-check" style="margin-right:5px;"></i>Верифицировать</button>' +
            '<button onclick="adminKycDecide(' +
            realIdx +
            ',\'rejected\')" style="flex:1;background:rgba(248,113,113,0.1);color:#f87171;border:1px solid rgba(248,113,113,0.3);border-radius:10px;padding:9px;font-weight:800;font-size:0.82rem;cursor:pointer;transition:0.2s;" onmouseover="this.style.background=\'rgba(248,113,113,0.2)\'" onmouseout="this.style.background=\'rgba(248,113,113,0.1)\'"><i class="fa-solid fa-xmark" style="margin-right:5px;"></i>Отклонить</button>' +
            "</div>"
          : "";
      return (
        '<div style="' +
        ...
        r.userId +
        "</div>" +
        '<div style="color:#94a3b8;font-size:0.75rem;margin-top:2px;">Документ: <b style="color:#fff;">' +
        (docNames[r.docType] || r.docType) +
        "</b> · ДР: " +
        r.dob +
        "</div>" +
...
function kycAdminPhoto(src, label) {
  if (!src)
    return (
      '<div style="background:rgba(100,116,139,0.08);border:1px dashed rgba(100,116,139,0.2);border-radius:10px;aspect-ratio:4/3;display:flex;align-items:center;justify-content:center;color:#475569;font-size:0.72rem;">' +
      label +
      "</div>"
    );
  return (
    '<div style="border-radius:10px;overflow:hidden;border:1px solid rgba(255,255,255,0.06);position:relative;aspect-ratio:4/3;cursor:pointer;" onclick="openKycPhotoFull(\'' +
    src +
    '\')" title="Открыть полностью">' +
    '<img src="' +
    src +
    '" style="width:100%;height:100%;object-fit:cover;display:block;">' +
    '<div style="position:absolute;bottom:0;left:0;right:0;background:rgba(0,0,0,0.6);color:#fff;font-size:0.65rem;font-weight:600;text-align:center;padding:3px;">' +
    label +
    ' <i class="fa-solid fa-up-right-and-down-left-from-center" style="font-size:0.55rem;opacity:0.7;"></i></div>' +
    "</div>"
  );
}
...
function openKycPhotoFull(src) {
  ...
  modal.style.cssText =
    "position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.92);z-index:99999;display:flex;align-items:center;justify-content:center;cursor:zoom-out;padding:20px;box-sizing:border-box;";
  ...
  closeBtn.style.cssText =
    "position:absolute;top:20px;right:20px;width:40px;height:40px;border-radius:50%;background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.2);color:#fff;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:1.2rem;";
...
function adminRenderWithdrawals() {
  ...
  if (tabBadge) {
    tabBadge.style.display = pending.length > 0 ? "flex" : "none";
    tabBadge.textContent = pending.length;
  }
  ...
  if (!displayList.length) {
    container.innerHTML =
      '<div style="color:#475569;font-size:0.85rem;text-align:center;padding:40px;background:rgba(0,0,0,0.2);border-radius:14px;border:1px dashed rgba(100,116,139,0.2);"><i class="fa-solid fa-inbox" style="font-size:2rem;color:#334155;display:block;margin-bottom:10px;"></i>' +
      (showProcessedWithdrawals ? "Нет запросов" : "Нет активных запросов") +
      "</div>";
    return;
  }
  ...
  container.innerHTML = reversed
    .map(function (r) {
      ...
      var statusBadge =
        r.status === "pending"
          ? '<span style="background:rgba(251,191,36,0.12);color:#fbbf24;border:1px solid rgba(251,191,36,0.3);border-radius:8px;padding:3px 10px;font-size:0.72rem;font-weight:700;">\u23f3 Ожидает</span>'
          : r.status === "approved"
            ? '<span style="background:rgba(74,222,128,0.12);color:#4ade80;border:1px solid rgba(74,222,128,0.3);border-radius:8px;padding:3px 10px;font-size:0.72rem;font-weight:700;">\u2714 Выполнено</span>'
      ...
      var btns =
        r.status === "pending"
          ? '<div style="display:flex;gap:8px;margin-top:12px;"><button onclick="adminWithdrawDecide(' +
            idx +
            ',\'approved\')" style="flex:1;background:rgba(74,222,128,0.15);color:#4ade80;border:1px solid rgba(74,222,128,0.35);border-radius:10px;padding:9px;font-weight:800;font-size:0.82rem;cursor:pointer;"><i class="fa-solid fa-check" style="margin-right:5px;"></i>Подтвердить</button><button onclick="adminWithdrawDecide(' +
            idx +
            ',\'rejected\')" style="flex:1;background:rgba(248,113,113,0.1);color:#f87171;border:1px solid rgba(248,113,113,0.3);border-radius:10px;padding:9px;font-weight:800;font-size:0.82rem;cursor:pointer;"><i class="fa-solid fa-xmark" style="margin-right:5px;"></i>Отклонить</button></div>'
          : "";
      return (
        '<div style="background:rgba(9,14,28,0.8);border:1px solid ' +
        (r.status === "pending" ? "rgba(251,191,36,0.2)" : r.status === "approved" ? "rgba(74,222,128,0.15)" : "rgba(248,113,113,0.15)") +
        ';border-radius:16px;padding:18px;"><div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;flex-wrap:wrap;"><div><div style="color:#fff;font-weight:700;font-size:0.95rem;">' +
        r.userName +
        ' <span style="color:#38bdf8;font-size:0.8rem;">ID: ' +
        r.userId +
        '</span></div><div style="color:#64748b;font-size:0.75rem;margin-top:3px;"><b style="color:#fff;">' +
        r.amount.toFixed(2) +
        "$ USDT</b> \u2192 " +
        ...
        r.network +
        ')</span></div><div style="color:#475569;font-size:0.72rem;margin-top:2px;">' +
        r.createdAt +
        "</div></div><div>" +
        statusBadge +
        "</div></div>" +
...
function adminRenderSupport() {
  ...
  if (tabBadge) {
    tabBadge.style.display = open.length > 0 ? "flex" : "none";
    tabBadge.textContent = open.length;
  }

  if (!tickets.length) {
    dialogsEl.innerHTML =
      '<div style="color:#475569;text-align:center;padding:40px;font-size:0.82rem;"><i class="fa-solid fa-inbox" style="font-size:1.5rem;color:#334155;display:block;margin-bottom:8px;"></i>Нет обращений</div>';
    return;
  }
  dialogsEl.innerHTML = tickets
    .slice()
    .reverse()
    .map(function (t, revIdx) {
      ...
      msgs.forEach(function (m) {
        if (m.from === "user" && new Date(m.time).getTime() > adminLastSeen) unread++;
      });
      var initials = t.userName
        ? t.userName
            .split(" ")
            .map(function (w) {
              return w[0] || "";
      ...
      return (
        ...
        idx +
        ')" style="padding:12px 16px;cursor:pointer;border-bottom:1px solid rgba(255,255,255,0.03);background:' +
        (isActive ? "rgba(37,99,235,0.1)" : "transparent") +
        ';transition:0.15s;display:flex;align-items:center;gap:10px;" onmouseover="this.style.background=\'rgba(255,255,255,0.03)\'" onmouseout="this.style.background=\'' +
        (isActive ? "rgba(37,99,235,0.1)" : "transparent") +
        "'\">" +
        '<div style="width:36px;height:36px;border-radius:50%;background:linear-gradient(135deg,#2563eb,#6366f1);display:flex;align-items:center;justify-content:center;font-weight:800;font-size:0.72rem;color:#fff;flex-shrink:0;">' +
        initials +
        "</div>" +
        '<div style="flex:1;min-width:0;">' +
        '<div style="display:flex;justify-content:space-between;align-items:center;"><span style="color:#fff;font-weight:700;font-size:0.82rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">' +
        t.userName +
        "</span>" +
        (unread > 0
          ? '<span style="background:#f87171;color:#fff;border-radius:50%;min-width:18px;height:18px;font-size:0.6rem;font-weight:900;display:flex;align-items:center;justify-content:center;flex-shrink:0;">' +
            unread +
            "</span>"
          : '<span style="color:#475569;font-size:0.65rem;flex-shrink:0;">' + t.createdAt.split(",")[0] + "</span>") +
        "</div>" +
        '<div style="color:#64748b;font-size:0.72rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:2px;">' +
        lastText +
        "...</div>" +
        "</div></div>"
      );
...
function openAdminChat(idx) {
  ...
  headerEl.innerHTML =
    '<div style="display:flex;align-items:center;gap:10px;"><div style="width:32px;height:32px;border-radius:50%;background:linear-gradient(135deg,#2563eb,#6366f1);display:flex;align-items:center;justify-content:center;font-weight:800;font-size:0.68rem;color:#fff;">' +
    t.userName
      .split(" ")
      .map(function (w) {
        return w[0] || "";
      ...
      .slice(0, 2)
      .toUpperCase() +
    '</div><div><div style="color:#fff;font-weight:700;font-size:0.88rem;">' +
    t.userName +
    '</div><div style="color:#64748b;font-size:0.7rem;">' +
    t.email +
    " · #" +
    t.id +
    "</div></div></div>" +
    ...
    (t.status === "open"
      ? '<button onclick="adminCloseTicket(' +
        idx +
        ')" style="background:rgba(74,222,128,0.1);color:#4ade80;border:1px solid rgba(74,222,128,0.25);border-radius:8px;padding:6px 12px;font-size:0.72rem;font-weight:700;cursor:pointer;"><i class="fa-solid fa-check" style="margin-right:4px;"></i>Закрыть</button>'
      : '<span style="color:#4ade80;font-size:0.72rem;">Закрыт</span>') +
    "</div>";

  var msgsEl = document.getElementById("adminChatMessages");
  var msgs = t.messages || [];
  var msgsHtml =
    '<div style="text-align:center;margin-bottom:12px;"><span style="background:rgba(100,116,139,0.1);color:#64748b;border:1px solid rgba(100,116,139,0.15);border-radius:20px;padding:4px 14px;font-size:0.7rem;">' +
    t.reason +
    ": " +
  ...
  msgsHtml += msgs
    .map(function (m) {
      ...
      return (
        ...
        ';margin-bottom:8px;"><div style="max-width:75%;background:' +
        (isAdmin ? "linear-gradient(135deg,rgba(37,99,235,0.15),rgba(59,130,246,0.1))" : "rgba(255,255,255,0.04)") +
        ";border:1px solid " +
        (isAdmin ? "rgba(59,130,246,0.25)" : "rgba(255,255,255,0.06)") +
        ';border-radius:12px;padding:10px 14px;"><div style="color:#fff;font-size:0.84rem;line-height:1.5;">' +
        m.text +
        '</div><div style="color:#475569;font-size:0.62rem;margin-top:4px;">' +
        senderName +
        " · " +
        m.time +
        "</div></div></div>"
  ...
  input.onkeydown = function (e) {
    if (e.key === "Enter") adminSendChat(idx);
  };
...
function checkUserSupportMessages() {
  ...
  var hasTickets = tickets.some(function (t) {
...
function openUserSupportChat() {
  ...
  var msgsHtml = allMsgs
    .map(function (m) {
      if (m.from === "system") {
        return (
          '<div style="text-align:center;margin:12px 0;"><span style="background:rgba(100,116,139,0.1);color:#64748b;border:1px solid rgba(100,116,139,0.2);border-radius:20px;padding:4px 14px;font-size:0.72rem;">' +
          m.text +
          " · " +
          m.time +
          "</span></div>"
      ...
      return (
        '<div style="display:flex;justify-content:' +
        (isAdmin ? "flex-start" : "flex-end") +
        ';margin-bottom:10px;"><div style="max-width:80%;background:' +
        (isAdmin ? "linear-gradient(135deg,rgba(251,191,36,0.1),rgba(245,158,11,0.06))" : "linear-gradient(135deg,rgba(37,99,235,0.15),rgba(59,130,246,0.1))") +
        ";border:1px solid " +
        (isAdmin ? "rgba(251,191,36,0.25)" : "rgba(59,130,246,0.25)") +
        ';border-radius:14px;padding:12px 16px;"><div style="color:#fff;font-size:0.85rem;line-height:1.6;">' +
        m.text +
        '</div><div style="color:#475569;font-size:0.65rem;margin-top:6px;display:flex;align-items:center;gap:6px;">' +
        (isAdmin ? '<i class="fa-solid fa-headset" style="color:#fbbf24;font-size:0.6rem;"></i> Поддержка' : '<i class="fa-solid fa-user" style="color:#38bdf8;font-size:0.6rem;"></i> Вы') +
        " · " +
        m.time +
        "</div></div></div>"
      );
  ...
  modal.style.cssText =
    "position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.85);z-index:99998;display:flex;align-items:center;justify-content:center;padding:20px;box-sizing:border-box;";
  modal.innerHTML =
    '<div style="background:linear-gradient(160deg,#0b1628,#060e1c);border:1px solid rgba(251,191,36,0.2);border-radius:20px;padding:0;max-width:500px;width:100%;max-height:85vh;display:flex;flex-direction:column;overflow:hidden;">' +
    ...
    '<div id="userChatMessages" style="flex:1;overflow-y:auto;padding:16px 20px;min-height:200px;">' +
    msgsHtml +
    "</div>" +
    '<div style="padding:14px 20px;border-top:1px solid rgba(255,255,255,0.06);display:flex;gap:8px;"><input type="text" id="userChatInput" placeholder="Написать сообщение..." style="flex:1;background:rgba(15,23,42,0.8);color:#fff;border:1px solid rgba(251,191,36,0.2);border-radius:12px;padding:11px 14px;font-size:0.85rem;outline:none;" onkeydown="if(event.key===\'Enter\')userSendChat(' +
    latestIdx +
    ')"><button onclick="userSendChat(' +
    latestIdx +
    ')" style="background:linear-gradient(135deg,#f59e0b,#d97706);color:#fff;border:none;border-radius:12px;padding:11px 18px;font-weight:800;font-size:0.85rem;cursor:pointer;"><i class="fa-solid fa-paper-plane"></i></button></div>' +
    "</div>";
  modal.onclick = function (e) {
    if (e.target === modal) modal.remove();
  };
...
function showSupportNotification() {
  ...
  toast.onclick = function () {
    toast.style.animation = "slideOutNotif 0.3s ease-in forwards";
    setTimeout(function () {
      toast.remove();
    }, 300);
    openUserSupportChat();
  ...
  if (iconEl) {
    if (coin === "TRS") {
      ...
      if (!trsIconEl) {
        ...
        trsIconEl.innerHTML =
          '<svg width="16" height="16" viewBox="0 0 24 24" fill="none"><polygon points="12,2 22,20 2,20" stroke="#fff" stroke-width="1.5" fill="rgba(255,255,255,0.15)"/><circle cx="12" cy="14" r="3" fill="#fff" opacity="0.9"/></svg>';
