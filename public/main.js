        ...
        if (authToken) {
            (async function loadUserFromServer() {
                ...
                if (result.ok && result.data.user) {
                    ...
                    if (user.avatar) {
                        localStorage.setItem('trustUserAvatar', user.avatar);
                    }
                    updateBalanceUI();
                    checkAuthUI();
                } else {
        ...
        async function fakeLogin() {
            ...
            localStorage.setItem('trustUserRole', user.role);
            if (user.emailVerified) localStorage.setItem('trustUserVerified', 'true');
            if (user.avatar) localStorage.setItem('trustUserAvatar', user.avatar);
        ...
        async function fakeReg() {
            ...
            showCustomAlert('Успешно!', 'Аккаунт успешно создан! Теперь войдите в свой аккаунт.', 'success');
            ...
            (async function() {
                ...
                if (!result.ok) {
                    showCustomAlert('Ошибка', result.data.error || 'Ошибка регистрации', 'error');
                }
            })();
        }
        ...
        function handleAvatarUpload(event) {
            ...
            reader.onload = function(e) {
                ...
                img.onload = async function() {
                    ...
                    localStorage.setItem('trustUserAvatar', compressed);
                    refreshSettingsAvatar();
                    // Save to server
                    if (authToken) {
                        await apiCall('/user/avatar', 'POST', { avatar: compressed });
                    }
                };
